...
export const ButtonLg = (props:Props) => {
    return (
        <button className={`disabled:fill-skin-muted disabled:text-skin-muted px-3 flex space-x-2 items-center  rounded cursor-pointer input-h no-dragable ${props.active ?  props.mode !== 'tab' ? 'bg-skin-fill-inverted text-skin-inverted fill-skin-inverted' : 'active-mode' : 'bg-hover-transparent'} no-dragable `} {...props.buttonProps}>
            {props.prefix && (
