...
export default function PointR({item,shadow,dim,layoutClass}: HomeDesqProps) {
  return (
    <Layout
        item={item}
        shadow={shadow}
    ...
    >
        <>
            <div className='flex-1 flex flex-col justify-center relative z-10 pl-[28px] pr-[10px]'>
                <Title title={item.title} domainColor={item.color2} domain={item.domain} />
                <Description text={item.description} />
            </div>
            <div className="w-[60%] h-full " style={{clipPath: templates.pointR}}>
                {!item.image ? (
